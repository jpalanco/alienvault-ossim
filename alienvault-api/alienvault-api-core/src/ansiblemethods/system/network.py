...
from ansiblemethods.system.system import get_av_config, set_av_config, ansible_is_gvm_installed, ansible_restart_gvm
...
def set_iface_promisc_status(system_ip, iface, status):
    ...
    response = ansible.run_module(
        host_list=[system_ip],
        module='command',
        args='/sbin/ifconfig %s %s' % (iface, flag)
    )
    # Check error
    if system_ip in response['dark']:
...
def get_conf_network_interfaces(system_ip, root=None, store_path=False):
    ...
    try:
        response = ansible.run_module(
            host_list=[system_ip],
            module="av_augeas",
            args="commands='match /files/etc/network/interfaces/iface/*  match /files/etc/network/interfaces/iface' validate_filepath=no"
        )
        if system_ip in response['dark']:
...
def delete_conf_iface(system_ip, iface, ifacepath=None):
    ...
    response = ansible.run_module(
        host_list=[system_ip],
        module="av_augeas",
        args="commands='rm %s  rm /files/etc/network/interfaces/auto/*[.=\\\"%s\\\"] rm /files/etc/network/interfaces/auto[count(*)=0]'  validate_filepath=no" % (ifacepath, iface)
    )
    if system_ip in response['dark']:
        return False, response['dark'][system_ip]['msg']
    elif response['contacted'][system_ip].get('failed',False) == True:
...
def make_tunnel(system_ip, local_server_id, password=""):
    ...
    try:
        ...
        # 3 - Copy the client configuration to its destination
        ...
        if "server" in remote_profiles:
            ...
            success, msg = ansible_is_valid_response(system_ip, response)
            if not success:
                api_log.error("Cannot restart the ossim-server service in the remote AlienVault appliance. {0}".format(msg))
        ...
        success, data = ansible_is_gvm_installed(system_ip=system_ip, remote_user="root", remote_password=password)
        if not success:
            error_msg = "Unable to check if GVM is installed in {0}".format(system_ip)
            api_log.error(error_msg)
            return False, error_msg

        gvm_packages = data["contacted"][system_ip]["stdout"].split("\n")
        if len(gvm_packages) > 1:
            print "Restarting remote GVM service ..."
            success, data = ansible_restart_gvm(system_ip=system_ip, remote_user="root", remote_password=password)
            if not success:
                error_msg = "Cannot restart the GVM service in {}: {}".format(system_ip, data)
                api_log.error(error_msg)
                return False, error_msg
        ...
        success, msg = ansible_is_valid_response(host, response)
        if not success:
            api_log.error("Cannot restart the ossim-server service. {0}".format(msg))
...
def set_interfaces_roles(system_ip, interface_roles):
    """ Check the role of subset of interfaces in the system

        @param system_ip      The system IP where we're going to operate
        @param interface_roles  A json describing each interface we're going
                              to touch
    ...
    admin_interfaces_future_net_status = [
        iface for iface, data in future_net_status.iteritems() if data['role'] == 'admin'
    ]
    ...
    ip_interfaces = [
        data['ipv4']['address'] for iface, data in future_net_status.iteritems() if
        ...
        if role == "log_management":
            ...
            if iface_address is None:
                result_ifaces[iface] = (
                    ...
                    "interface we need an IP address(%s)" % (iface, iface_address)
                )
                continue
            if iface_netmask is None:
                result_ifaces[iface] = (
                    False,
                    "In order to configure the given interface (%s) as a log management "
                    "interface we need a valid netmask (%s)" % (iface, iface_netmask)
                )
                continue
