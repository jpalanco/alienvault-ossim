...
#
"""
    Methods to use the av_center_facts functionality
"""
from collections import namedtuple
import api_log
from ansiblemethods.ansiblemanager import Ansible
from ansiblemethods.helper import parse_av_config_response, read_file, ansible_is_valid_response, package_list_generic
    ...
    """
     Get the alienvault_status system info
        ...
        Returns:
            ...
            the all status data if the execution is correct.
    """
...
def system_status(system_ip):
    ...
    if system_ip in response['contacted']:
        ...
        if not response['contacted'][system_ip].get('failed', False):
            ...
            api_log.error("Error calling module 'av_center_facts sections=system_status' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
            result = APIResult(False, "Error calling module 'av_center_facts sections=system_status' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
    else:
        api_log.error("Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
        result = APIResult(False, "Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
    return result
...
def network_status(system_ip):
    ...
    if system_ip in response['contacted']:
        ...
        if not response['contacted'][system_ip].get('failed', False):
            ...
            api_log.error("Error calling module 'av_center_facts sections=network_status' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
            result = APIResult(False, "Error calling module 'av_center_facts sections=network_status' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
    else:
        api_log.error("Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
        result = APIResult(False, "Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
    return result
...
def alienvault_status(system_ip):
    ...
    response = ansible.run_module(
        host_list=[system_ip],
        module="av_center_facts",
        args="sections=alienvault_status"
    )
...
def cpu(system_ip):
    ...
    if system_ip in response['contacted']:
        ...
        if not response['contacted'][system_ip].get('failed', False):
            ...
            api_log.error("Error calling module 'av_center_facts sections=cpu' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
            result = APIResult(False, "Error calling module 'av_center_facts sections=cpu' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
    else:
        api_log.error("Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
        result = APIResult(False, "Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
    return result
...
def get_local_time(system_ip, date_fmt=None):
    ...
    if system_ip in response['contacted']:
        if not response['contacted'][system_ip].get('failed', False):
            ...
            result = (
                False, "Error getting local time: %s" % response['contacted'][system_ip].get('msg', 'Unknown error')
            )
    else:
...
def disk_usage(system_ip):
    ...
    if system_ip in response['contacted']:
        ...
        if not response['contacted'][system_ip].get('failed', False):
            ...
            api_log.error("Error calling module 'av_center_facts sections=disk_usage' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
            result = APIResult(False, "Error calling module 'av_center_facts sections=disk_usage' msg:%s" % response['contacted'][system_ip].get('msg', "Unknown error"))
    else:
        api_log.error("Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
        result = APIResult(False, "Can't connect to system with IP %s msg:%s " % (system_ip, str(response['dark'][system_ip])))
    return result
...
def package_list(system_ip, package_name, remote_user=None, remote_password=None):
    """
    Search for a package, for more information please look into package_list_generic

    Args:
        system_ip (str): String with a IP address.
        package_name (str): package looking for
        remote_user (str): remote user to perform actions
        remote_password (str): remote password to perform actions
    ...
    success, response = package_list_generic(system_ip, package_name, remote_user, remote_password)
    if not success:
        api_log.error(response)
        return False, "Something wrong happened getting the packages list: %s" % response
    ...
    if response['contacted'][system_ip]['stdout'] != "":
        ...
        for p in packages:
            p = p.split("@@@")
            pck = {'version': p[1].split("-")[0], 'description': p[2]}
            result[p[0]] = pck
