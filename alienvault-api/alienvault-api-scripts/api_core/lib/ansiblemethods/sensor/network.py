...
def get_pfring_stats (sensor_ip):
    """Retrieves pfring stats file.
    :param systemp_ip (String dotted)
    """
    return ansible.run_module(host_list=[sensor_ip], module="av_pfring",args="")
...
def get_sensor_networks (sensor_ip):
    ...
    rc = False
    networks=[]
    response = ansible.run_module(host_list=[sensor_ip], module="av_config",args="sensor_networks=True op=get")
    # Verify the response
    if sensor_ip in response['dark']:
        networks = response['dark'][sensor_ip].get('msg',"Unknown  Ansible error")
    elif response['contacted'][sensor_ip].get('failed',False) == True:
        networks = response['contacted'][sensor_ip].get('msg',"Unknown error from Ansible Module av_config")
    else:
        # Ok, wr have the networks
...
def set_sensor_networks(sensor_ip,nets=[]):
    ...
    # Join
    args = ",".join(nets)
    response =  ansible.run_module(host_list=[sensor_ip],module="av_config",args="sensor_networks=%s op=set" % args)
    if sensor_ip in response['dark']:
