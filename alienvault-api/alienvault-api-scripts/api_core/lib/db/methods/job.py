...
def update_job_data(job_id, data):
    try:
        # FIXME: this launch multiple times the same query, why?
        #db.session.execute("REPLACE INTO alienvault_api.celery_job (id, info) VALUES (:id, :info)", {'id': job_id, 'info': data})
        db.session.connection(mapper=Celery_Job).execute("REPLACE INTO alienvault_api.celery_job (id, info) VALUES (0x%s, 0x%s)" % (binascii.hexlify(job_id), binascii.hexlify(data)))
    except Exception:
        db.session.rollback()
        return False
    return True
