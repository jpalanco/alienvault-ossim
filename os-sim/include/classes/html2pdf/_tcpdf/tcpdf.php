	...
	class TCPDF {
		...
		*/
		public function SetLineStyle($style) {
			...
			// EXTR_SKIP fixes the Possible Variable Overwrite
			extract($style, EXTR_SKIP);
		...
		*/
		public function setSignature($signing_cert='', $private_key='', $private_key_password='', $extracerts='', $cert_type=2, $info=array()) {
			...
			if (strlen($signing_cert) == 0) {
				...
				// Password hardcored fortify alert
			...
			$this->signature_data['password'] = (strlen($signing_cert) == 0) ? "tcpdfdemo" : $private_key_password;
										...
										switch ($strpiece[2][0]) {
											...
											case 're': {
												...
												$pmid = preg_replace_callback('/('.$xmatches[1].')[\s]('.$xmatches[2].')[\s]('.$xmatches[3].')[\s]('.$strpiece[1][0].')[\s](re)([\s]*)/x',
													...
													$newx = sprintf("%.2F",(floatval($matches[1]) + '.floatval($x_diff).'));
													$neww = sprintf("%.2F",(floatval($matches[3]) + '.floatval($w_diff).'));
