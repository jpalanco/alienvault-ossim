...
class styleHTML
{
	...
	*/
	...
	public function ConvertToColor($val, &$res)
	{
		...
		{
			$r = $this->ConvertSubColor($match[1])*255.;
			$v = $this->ConvertSubColor($match[2])*255.;
			$b = $this->ConvertSubColor($match[3])*255.;
			$col = array($r, $v, $b);
		}
		elseif (preg_match('/cmyk\([\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*\)/isU', $val, $match))
		{
			$c = $this->ConvertSubColor($match[1])*100.;
			$m = $this->ConvertSubColor($match[2])*100.;
			$y = $this->ConvertSubColor($match[3])*100.;
			$k = $this->ConvertSubColor($match[4])*100.;
			$col = array($c, $m, $y, $k);
		}
		else
		{
