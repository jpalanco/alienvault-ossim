...
class IDS_Filter_Storage
{
    ...
    public final function __construct(IDS_Init $init) 
    {
        if ($init->config) {
            ...
            $this->source_xss = $init->getBasePath() 
                . $init->config['General']['xss_path'];
    ...
    public final function getFilterXSS() 
    {
    ...
    public final function addFilterXSS(IDS_Filter $filter) 
    {
    ...
    public function getFilterFromXML() 
    {
        ...
        if (extension_loaded('SimpleXML')) {
             ...
             */
            $filters = $filters_xss = $this->_isCached();
            ...
            if (!$filters_xss) {   
                if (file_exists($this->source_xss)) {
                    if (LIBXML_VERSION >= 20621) {
                        $filters_xss = simplexml_load_file($this->source_xss,
                                                       null,
                                                       LIBXML_COMPACT);
                    } else {
                        $filters_xss = simplexml_load_file($this->source_xss);
             ...
             * will be thrown
             */
            if (empty($filters) || empty($filters_xss)) {
                throw new Exception(
            ...
            $data_xss    = array();
            $nocache     = $filters_xss instanceof SimpleXMLElement;
            $filters_xss = $nocache ? $filters_xss->filter : $filters_xss;

            foreach ($filters_xss as $filter) {
                ...
                $this->addFilterXSS(new IDS_Filter($id,
                                                $rule,
                                                $description,
                                                (array) $tags[0],
                                                (int) $impact));
