...
class IDS_Monitor
{
    ...
    public function __construct(array $request, IDS_Init $init, array $tags = null)
    {
        ...
        if (!empty($request)) {
			...
			$this->bypass = isset($init->config['General']['bypass'])
                ? $init->config['General']['bypass'] : false;
    ...
    private function _detect($key, $value)
    {
		...
		// check if this field is part of the exceptions
        $filter_bypass = false;
        if (is_array($this->bypass)) {
            foreach($this->bypass as $bypass) {
                $matches = array();
                if(preg_match('/(\/.*\/[^eE]*)$/', $bypass, $matches)) {
                    if(isset($matches[1]) && preg_match($matches[1], $key)) {
                        //return false;
                        $filter_bypass = true;
                    } 
        ...
        elseif ($xss_exception) {
            // Load xss filters
            $filterSet = $this->storage->getFilterXSS();
        }
		else {
            // load all filters
