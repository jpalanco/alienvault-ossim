...
$validate = array("locations_id" => array("validation" => "OSS_HEX, OSS_NULLABLE", "e_message" => 'illegal:' . _("ID")), "ctx" => array("validation" => "OSS_HEX, OSS_NULLABLE", "e_message" => 'illegal:' . _("CTX")), "l_name" => array("validation" => "OSS_ALPHA, OSS_SPACE, OSS_SCORE, OSS_DOT", "e_message" => 'illegal:' . _("Name")), "desc" => array("validation" => "OSS_TEXT, OSS_NULLABLE", "e_message" => 'illegal:' . _("Description")), "search_location" => array("validation" => "OSS_TEXT", "e_message" => 'illegal:' . _("Location")), "latitude" => array("validation" => "OSS_DIGIT, '\.\-', OSS_NOTNULL", "e_message" => 'illegal:' . _("Latitude")), "longitude" => array("validation" => "OSS_DIGIT, '\.\-', OSS_NOTNULL", "e_message" => 'illegal:' . _("Longitude")), "country" => array("validation" => "OSS_LETTER, OSS_NULLABLE", "e_message" => 'illegal:' . _("Country")), "sensor_list" => array("validation" => "OSS_HEX, OSS_NULLABLE", "e_message" => 'illegal:' . _("Sensor List")),);
...
if (GET('ajax_validation') == TRUE) {
	...
	if (is_array($validation_errors) && !empty($validation_errors)) {
		...
		//Checking max name length
		switch ($_GET['name']) {

			case 'l_name':

				$l_name = GET($_GET['name']);
				if (mb_strlen($l_name) > Locations::$metadata["name"]["max_length"]) {
					$data['status'] = 'error';
					$data['data']['l_name'] = _('Name is too long');
				}
...
//Extend validation to check name max length
if (empty($validation_errors['l_name'])) {

	if (isset($_POST['l_name']) && !empty($_POST['l_name'])) {
		$l_name = POST('l_name');
		if (mb_strlen($l_name) > Locations::$metadata["name"]["max_length"]) {
			$validation_errors['l_name'] = _('Name is too long');
		}
	}
