...
This one is modified by me, Javier Martinez Navajas. It is not the same than the one in js/jquery.editinplace.js so
...
$.extend(InlineEditor.prototype, {
	...
	connectOpeningEvents: function() {
		...
		this.dom
			...
			.bind('mousedown.editInPlace', function(anEvent){ if(anEvent.which==3)that.openEditor(anEvent); });
	},
	...
	handleCancelEditor: function(anEvent) {
		...
		if (hasContent(enteredText) 
			...
			//this.setClosedEditorContent(enteredText);  //It is not working properly
	...
	handleSaveEditor: function(anEvent) {
		...
		var enteredText = trim(this.dom.find(':input').val());
		enteredText = this.triggerDelegateCall('willCloseEditInPlace', enteredText);
