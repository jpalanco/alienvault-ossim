...
if (Session::menu_perms($m_perms, $sm_perms))
{
    ...
    try
    {
        ...
        if (!array_key_exists($agent_id, $agents)){
            $error_msg = _('Error! Not enough permissions to retrieve the HIDS agent information.');
            Av_exception::throw_error(Av_exception::USER_ERROR, $error_msg);
        }
        ...
        if (is_array($more_info) && !empty($more_info))
        {
            ...
            if ($agent_ip == '0.0.0.0/0' || $agent_ip == 'any') {
                $last_scan_dates = Ossec_agent::get_last_scans($sensor_id, $agent_id);
            }

            $syscheck_date   = (empty($last_scan_dates['syscheck']))  ? $more_info[6] : $last_scan_dates['syscheck'];
            $rootcheck_date  = (empty($last_scan_dates['rootcheck'])) ? $more_info[7] : $last_scan_dates['rootcheck'];
