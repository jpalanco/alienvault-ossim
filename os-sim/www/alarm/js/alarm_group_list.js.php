...
/*
 *    Check if action buttons should be enabled or disabled
 */
 ...
 */
function check_background_tasks(times, retries) {
            ...
            success  : function(data)
            {
                ...
                else
                {
                    if(data.bg)
                    {
                        ...
                        setTimeout(function()
                        {
                            check_background_tasks(times+1, retries);
                    ...
                    else
                    {
                        if(__flag_bg)
                        {
                            ...
                            //Parameter is set if an action was performed
                            if (retries != -1)
                            {
                                // Background task was executed but we cannot get PID yet
                                if (retries > 0){
                                    setTimeout(function()
                                    {
                                        check_background_tasks(times, retries - 1);
...
function change_group_status() {
    ...
    av_confirm(msg, keys).done(function()
    {
        ...
        change_group_status_action(new_status, groups, elem);
    });
}
 ...
 */
function change_group_status_action(status, groups, elem) {
    var params = {};
    var data   = {};
    ...
    $.ajax(
        {
            ...
            success  : function(data)
            {
                ...
                {
                    check_background_tasks(0, __retries);
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown)
            {
