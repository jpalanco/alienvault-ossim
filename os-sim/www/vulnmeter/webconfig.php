...
$cvta             = intval(POST("close_vuln_tickets_automatically"));
...
else if( $action == "create" ) // Create credentials
{
    ...
    if($error_message == "")
	{
        ...
        $node = $doc->createElement('name', base64_encode($name));
    ...
    else
	{
        ...
        if($conn->Execute ( $sql, $params ) === false) {
            $error_message = _('Error updating settings: Portal branding cannot be updated on the database') . '<br/>';
        }
    }
    ...
    else
	{
	    ...
	    if ($file_type == 'image')
	    {
            ...
            if($conn->Execute ( $sql, $params ) === false) {
                $error_message .= _('Error updating settings: Site header logo cannot be updated on the database') . '<br/>';
            }
	    }
	    else
	    {
    ...
    if($conn->Execute ($sql, $params) === false) {
        $error_message .= _('Error updating settings: Vulnerability Ticket Threshold cannot be updated on the database') . '<br/>';
    }

    $sql = "UPDATE config SET value = ? WHERE conf = 'close_vuln_tickets_automatically'";
    $params = array($cvta);

    if($conn->Execute ($sql, $params) === false) {
        $error_message .= _('Error updating settings: Close Tickets Automatically cannot be updated on the database') . '<br/>';
    }
...
$cvta         = $conn->GetOne("SELECT value FROM config WHERE conf = 'close_vuln_tickets_automatically'");
...
<head>
    ...
    <script type="text/javascript">
        function delete_credential(key){
            var msg  = "<?php echo Util::js_entities(_('Are you sure you want to delete this credential?')) ?>";
            var opts = {"yes": "<?php echo _('Yes') ?>", "no": "<?php echo _('No') ?>"};

            av_confirm(msg, opts).fail(function(){
                return false;
            }).done(function(){
                document.location.href= "<?php echo AV_MAIN_PATH ?>/vulnmeter/webconfig.php?action=delete&id="+key;
            });
        }
		...
		$(document).ready(function(){
           ...
           $('#help_tooltip_cvta').tipTip({maxWidth: '600px', attribute: 'data-cvta', defaultPosition: 'top'});
		});
...
<body>
    ...
    <table width="90%" class="transparent" cellspacing="0" cellpadding="0" id="maintable">
        <tr>
            <td width="40%">
						...
						foreach($list as $item)
						{
							...
							<tr>
								...
								<td>
									...
									{
										?>
										<a id="delete_credential_<?php echo $credential_id ?>" href="javascript:;" style="margin-left:5px;text-decoration:none" onclick="delete_credential('<?php echo $credential_id ?>')">
            ...
            <td width="55%" valign="top">
				...
				<form method='post' action='webconfig.php'>
					...
					<table class="w100">
                        ...
                        <tr>
                            <td><?php echo _("Close Tickets Automatically:") ?></td>
                            ...
                            <td class='cvta_td'>
                                ...
                                $cvta_tooltip = _('Tickets related to vulnerabilities will be automatically closed if the vulnerabilities found in the previous scans are not present in the latest scan.').'<br/>'.
                                    _('This option could cause false positives if the scans are not executed under the same conditions.  Therefore, watch out for the following cases:').'<br/>
                                    <ul>
                                ...
                                ?>
                                <select name="close_vuln_tickets_automatically">
                                    <?php
                                    foreach ($cvta_values as $cvta_label => $cvta_value)
                                    {
                                        ?>
                                        <option <?php echo $cvta_value == $cvta ? " selected='selected'": ""?> value="<?php echo $cvta_value?>">
                                            <?php echo $cvta_label?>
                                        </option>
                                        <?php
                                    }
                                ...
                                <img id='help_tooltip_cvta' data-cvta="<?php echo $cvta_tooltip?>" src="/ossim/pixmaps/help_small.png"/>
