...
{
    //The ctx will be ignored in this version but the function is built thinking of the future...
    $ctx = (!empty($data['ctx'])) ? $data['ctx'] : Session::get_default_ctx();
	$plugin_id = (!empty($data['plugin_id'])) ? $data['plugin_id'] : NULL;
	...
	ossim_valid($ctx,           OSS_HEX,                        'illegal:' . _("CTX"));
    ossim_valid($plugin_id,     OSS_DIGIT, OSS_NULLABLE,        'illegal:' . _("Plugin ID"));
    ossim_valid($product_types, OSS_DIGIT, "\,'", OSS_NULLABLE, 'illegal:' . _("Product Type"));
    ...
    if (!empty($plugin_id)) {
        $params[] = $plugin_id;
        $query .= " WHERE id IN (SELECT DISTINCT category_id FROM plugin_sid where plugin_id = ?)";

    } else if (!empty($product_types)) {
        $query .= " WHERE id IN (SELECT DISTINCT category_id FROM plugin_sid ps JOIN plugin p 
                    ON ps.plugin_id = p.id WHERE p.product_type IN ($product_types))";
    }
...
function get_subcategories($conn, $data)
{
	...
	ossim_valid($ctx,           OSS_HEX,		                'illegal:' . _("CTX"));
	ossim_valid($cat_id,        OSS_HEX, OSS_NULLABLE,		    'illegal:' . _("Category ID"));
    ossim_valid($plugin_id,     OSS_DIGIT, OSS_NULLABLE,        'illegal:' . _("Plugin ID"));
    ossim_valid($product_types, OSS_DIGIT, "\,'", OSS_NULLABLE, 'illegal:' . _("Product Type"));
    ...
    if (!empty($plugin_id)) {
        $params[] = $plugin_id;
        $query .= " AND id IN (SELECT DISTINCT subcategory_id FROM plugin_sid where plugin_id = ?)";

    } else if (!empty($product_types)) {
        $query .= " AND id IN (SELECT DISTINCT subcategory_id FROM plugin_sid ps JOIN plugin p 
                    ON ps.plugin_id = p.id WHERE p.product_type IN ($product_types))";
    }
