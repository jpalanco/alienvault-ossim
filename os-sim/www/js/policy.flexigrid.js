...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
				 	...
				 	function ()
						{
							...
							var rtype  = $(this).attr('col_type');
							if (rtype) tr.type = rtype;
			...
			populate: function () { //get latest data
				...
				if (g.rowCopy)
				{
					...
					$("#srcroworder").html('');
					$("#dstroworder").html('');
					$("#srcroworder").data('type','');
					$("#dstroworder").data('type','');
					g.drowp = null;
					g.hrow = null;
					g.hset = null;
					g.rowCopy = null;
								...
								function (e)
									{
										...
										$("#srcroworder").data('type', this.type);
								...
								function (e)
									{
										...
										if (g.rowCopy)
										{
											...
											{
												if($("#srcroworder").data('type') == $("#dstroworder").data('type'))
												{
											...
											if (p.onToggleGrid && $("#srcroworder").data('type') == $("#dstroworder").data('type')){
												p.onToggleGrid($("#srcrow").html(),$("#dstrow").html());
											}
							...
							.hover(
								function (e) 
									{ 
										...
										$("#dstroworder").data('type',this.type);
										//if (p.contextMenu) $(".contextMenu").fadeOut(p.outSpeed);
										if (g.rowCopy)
										{
										...
										else if ($("#srcrow").html()!='')
										{
											...
											{
												$("#rowCopy").html('<b>You are not allowed to swap policies between contexts and engines.</b>');
											}
										}
									},
								function () {}
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									nl = onl - nw + Math.floor(p.cgwidth/2) + 10;
		...
		if ($.browser.msie&&$.browser.version<7) $('.pButton',g.pDiv).hover(function(){$(this).addClass('pBtnOver');},function(){$(this).removeClass('pBtnOver');});
			...
			$('.pDiv2',g.pDiv).prepend("<div class='pGroup'><table><tr><td>Show rows: </td><td><select name='rp'>"+opt+"</select></td></tr></table></div> <div class='btnseparator'></div>");
			...
			if (p.onUpDown)
				{
					...
					$('span.up',g.mDiv).click( function (e) {
						p.onUpDown(p,'up');
						e.stopPropagation();
					});
					$('span.down',g.mDiv).click( function (e) {
						p.onUpDown(p,'down');
						e.stopPropagation();
					});
				}
				...
				function (e)
					{
						...
						if ($("#srcrow").html()!='' && $("#rowCopy").html()!='' ) {
							...
							if($("#srcroworder").data('type') == p.typeGroup)
							{
				...
				function (e)
					{
						...
						if ($("#srcrow").html()!='' && $("#dstrow").html()!='') {
							$("#rowCopy").css({display: 'none'});
							if (p.onToggleGrid && $("#srcroworder").data('type') == p.typeGroup)
							{
								p.onToggleGrid($("#srcrow").html(),$("#dstrow").html());
							}
							$("#srcrow").html('');
