...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		var removeAllContainer = ( this.options.remove_all ) ? '<a href="#" class="remove-all">'+$.tmpl($.ui.multiselect.locale.removeAll)+'</a>' : '';
		this.selectedActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">'+$.tmpl($.ui.multiselect.locale.itemsCount,{count:0})+'</span>'+removeAllContainer+'</div>').appendTo(this.selectedContainer);
		
		var addAllContainer = ( this.options.add_all ) ? '<a href="#" class="add-all">'+$.tmpl($.ui.multiselect.locale.addAll)+'</a>' : '';
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'+$.tmpl($.ui.multiselect.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"/>'+addAllContainer+'</div>').appendTo(this.availableContainer);
