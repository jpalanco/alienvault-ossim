...
(select 'Operational procedures and responsibilities', count(*) as volume from datawarehouse.ssi_user a,
datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.1.%') as A_10_1
UNION SELECT * FROM
(select 'Third party service delivery management', count(*) as volume from datawarehouse.ssi_user a,
datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.2.%') as A_10_2
UNION SELECT * FROM
(select 'System planning and acceptance', count(*) as volume from datawarehouse.ssi_user a,
datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.3.%') as A_10_3
UNION SELECT * FROM
(select 'Protection against malicious and mobile code', count(*) as volume from datawarehouse.ssi_user a,
datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.4.%') as A_10_4
UNION SELECT * FROM
(select 'Back-up', count(*) as volume from datawarehouse.ssi_user a, datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.5.%') as A_10_5
UNION SELECT * FROM
(select 'Network security management', count(*) as volume from datawarehouse.ssi_user a,
datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.6.%') as A_10_6
UNION SELECT * FROM
(select 'Media handling', count(*) as volume from datawarehouse.ssi_user a, datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
ISO27001An.A10_Com_OP_Mgnt) AND i.ref LIKE 'A.10.7.%') as A_10_7
UNION SELECT * FROM
(select 'Exchange of information', count(*) as volume from datawarehouse.ssi_user a, datawarehouse.iso27001sid i
where ".$sql_year." AND a.user = '".$user."' AND a.sid=i.sid and i.ref IN (SELECT ref from
...
$var=array();
while (!$rs->EOF) {
    $var1 = $rs->fields["Operational procedures and responsibilities"];
...
$graph->Set90AndMargin(280,10,20,0);
