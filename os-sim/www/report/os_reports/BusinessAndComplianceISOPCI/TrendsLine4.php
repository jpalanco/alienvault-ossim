...
(select 'Approach-Attack' as category, s.month as mois, sum(c.approach) as volume from datawarehouse.ssi_user s,
datawarehouse.category c
where s.year =".$year." AND s.user = '".$user."' AND s.sid=c.sid GROUP BY 1,2) as approach
UNION SELECT * FROM
(select 'Exploration-Attack' as category, s.month as mois, sum(c.exploration) as volume from
datawarehouse.ssi_user s, datawarehouse.category c
where s.year =".$year." AND s.user = '".$user."' AND s.sid=c.sid GROUP BY 1,2) as exploration
UNION SELECT * FROM
(select 'Penetration-Attack' as category, s.month as mois, sum(c.penetration) as volume from datawarehouse.ssi_user
s, datawarehouse.category c
where s.year =".$year." AND s.user = '".$user."' AND s.sid=c.sid GROUP BY 1,2) as penetration
UNION SELECT * FROM
(select 'General-Malware' as category, s.month as mois, sum(c.generalmalware) as volume from
datawarehouse.ssi_user s, datawarehouse.category c
where s.year =".$year." AND s.user = '".$user."' AND s.sid=c.sid GROUP BY 1,2) as generalmalware
) AS allalarms;";
...
foreach ($var as $value){
    if($value['var1']=='Approach-Attack'){
        $data1['value'][$value['var3']-1]=$value['var2'];
    }elseif($value['var1']=='Exploration-Attack'){
        $data2['value'][$value['var3']-1]=$value['var2'];
    }elseif($value['var1']=='Penetration-Attack'){
        $data3['value'][$value['var3']-1]=$value['var2'];
    }elseif($value['var1']=='General-Malware'){
